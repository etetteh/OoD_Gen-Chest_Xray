...
parser.add_argument('--filename', type=str, default="baseline_trainVal_results.csv", help='File name to save train and val results to')
...
parser.add_argument('--num_batches', type=int, default=10, help='')
...
if os.path.exists(cfg.filename):
    with open(cfg.filename, 'a+') as csvfile:
        ...
        for train_loss, val_loss, val_auc in zip(train_losses, valid_losses, valid_aucs):
            writer.writerow({'Train_loss': round(train_loss, 4), 
                         'Valid_loss': round(val_loss, 4), 
                         'Valid_AUC': round(val_auc, 4),
                         })
else:
    with open(cfg.filename, 'w') as csvfile:
        field_names = ['Train_loss', 'Valid_loss', 'Valid_AUC']
        writer = csv.DictWriter(csvfile, fieldnames=field_names)
