...
parser.add_argument('--output_dir', type=str, default="chest_xray_rex/")
...
parser.add_argument('--steps', type=int, default=1200, help='')
...
# get pathologies from each dataset
# nih = NIH_dataset.pathologies
...
datasets.default_pathologies = ['Effusion', 'Consolidation', 'Cardiomegaly', 'Edema']
...
def data_loaders(cfg, steps, train_datas, valid_data, test_data=None):  
    train_loaders = [[{} for i in range(steps)] for i in range(len(train_datas))]
...
#                            drop_last=True)    
    ...
    return train_loaders, valid_loader #, test_data

train_loaders, valid_loader = data_loaders(cfg=cfg, steps=cfg.steps, 
                                           train_datas=train_datas, valid_data=PC_dataset, test_data=None)
...
scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, 0.001, epochs=cfg.num_epochs, steps_per_epoch=cfg.steps)        
...
def train_epoch(steps, model, train_loaders, epoch, device, optimizer):
    model.train()
    avg_loss = []
    t = tqdm(range(1, steps+1))
    for step in t:
        ...
        penalty_weight = (cfg.penalty_weight 
          if step >= cfg.penalty_anneal_iters else 1.0)
...
def valid_test_epoch(name, epoch, model, device, data_loader, criterion, limit=cfg.steps):
    model.eval()
    ...
    return auc, torch.as_tensor(avg_loss).mean()
...
def rex_train(model, num_epochs, patience):        
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        avg_loss = train_epoch(steps=cfg.steps,
                               model=model,
...
metrics, best_metric, train_losses, valid_losses = rex_train(model=model, num_epochs=cfg.num_epochs, patience=cfg.patience)
