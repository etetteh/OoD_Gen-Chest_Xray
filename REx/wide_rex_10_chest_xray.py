...
parser.add_argument('--output_dir', type=str, default="wide_rex_10_chest_xray/")
...
train_loaders, valid_loader, test_loader = data_loaders(cfg=cfg, num_batches=cfg.num_batches, 
                                           train_datas=train_datas, valid_data=NIH_dataset, test_data=MIMIC_CH_dataset)
...
tv_filename = "wide_rex_10_" + cfg.model_name + "-" + str(cfg.batch_size) + "-" + str(cfg.num_batches) + "_trainVal_results.csv"
...
model.load_state_dict(torch.load("wide_rex_10_chest_xray/cx_pc-wide_resnet50_2-valid-best.pt").state_dict())
model = model.to(device)
...
test_filename = "wide_rex_10_" + cfg.model_name + "-" + str(cfg.batch_size) + "-" + str(cfg.num_batches) + "_test_results.csv"
