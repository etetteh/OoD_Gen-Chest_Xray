...
parser.add_argument('--output_dir', type=str, default="wide_baseline_5_chest_xray/")
...
test_loader = DataLoader(CHEX_dataset,
                       batch_size=cfg.batch_size,
                       shuffle=False,
                       num_workers=cfg.num_workers,
                       worker_init_fn=worker_init_fn, 
...
tv_filename = "wide_baseline_5_" + cfg.model_name + "-" + str(cfg.batch_size) + "-" + str(cfg.num_batches) + "_trainVal_results.csv"
...
###################################### Test ######################################
def inference(name, model, device, data_loader, criterion, limit=3*cfg.num_batches):
    model.eval()
...
model.load_state_dict(torch.load("wide_baseline_5_chest_xray/nih_mc-wideresnet50_2-valid-best.pt").state_dict())
model = model.to(device)
...
test_filename = "wide_baseline_5_" + cfg.model_name + "-" + str(cfg.batch_size) + "-" + str(cfg.num_batches) + "_test_results.csv"
