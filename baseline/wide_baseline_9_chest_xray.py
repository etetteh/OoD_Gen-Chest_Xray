...
parser.add_argument('--output_dir', type=str, default="wide_baseline_9_chest_xray/")
parser.add_argument('--model_name', type=str, default="wideresnet50_2")
parser.add_argument('--dataset_name', type=str, default="cx_pc")
...
valid_loader = DataLoader(MIMIC_CH_dataset,
                       batch_size=cfg.batch_size,
                       shuffle=False,
                       num_workers=cfg.num_workers,
                       worker_init_fn=worker_init_fn,
...
test_loader = DataLoader(NIH_dataset,
                       batch_size=cfg.batch_size,
                       shuffle=False,
                       num_workers=cfg.num_workers,
                       worker_init_fn=worker_init_fn, 
...
tv_filename = "wide_baseline_9_" + cfg.model_name + "-" + str(cfg.batch_size) + "-" + str(cfg.num_batches) + "_trainVal_results.csv"
...
model.load_state_dict(torch.load("wide_baseline_9_chest_xray/cx_pc-wideresnet50_2-valid-best.pt").state_dict())
model = model.to(device)
...
test_filename = "wide_baseline_9_" + cfg.model_name + "-" + str(cfg.batch_size) + "-" + str(cfg.num_batches) + "_test_results.csv"
...
with open(test_filename, 'w') as csvfile:
    ...
    writer.writerow({'Test_loss': round(test_loss, 4), 
                     ...
                     'Effusion': round(task_aucs[0], 4),
                     'Cardiomegaly': round(task_aucs[1], 4),
